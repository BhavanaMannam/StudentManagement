...
function GPA() {
  ...
  const fetchAll = async () => {
    const res = await axios.get("https://studentmanagement-306r.onrender.com/gpas");
    setGpas(res.data);
  };

  const saveGpa = async () => {
    await axios.post("https://studentmanagement-306r.onrender.com/gpa", { gpa });
    fetchAll();
  };

  const searchByGpa = async () => {
    const res = await axios.get(
      `https://studentmanagement-306r.onrender.com/gpa/search?value=${gpa}`
    );
    setGpas(res.data);
  };
  ...
  const filterRange = async () => {
    const res = await axios.get(
      `https://studentmanagement-306r.onrender.com/gpa/range?min=${min}&max=${max}`
    );
    setGpas(res.data);
  };
